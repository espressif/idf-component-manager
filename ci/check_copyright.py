...
MIME = {'python': 'text/x-python', 'c': 'text/x-c', 'cpp': 'text/x-c++'}
...
def contains_any_copyright(comments: list, args: argparse.Namespace) -> bool:
    ...
    """
    return any(
        comment.line_number() <= args.max_lines and re.search(r'copyright', comment.text(), re.IGNORECASE)
        for comment in comments)
...
def replace_copyright(code_lines: list, year: int, line: int, mime: str, file_name: str) -> list:
    ...
    print(
        f'{TERMINAL_GRAY}"{file_name}": replacing old header (lines: {line}-{end}) '
        f'with new SPDX header style.{TERMINAL_RESET}')
...
def check_copyrights(args: argparse.Namespace) -> Tuple[List, List, List]:
    """
        ...
        if file_name in ignore_list:
            ...
            if valid:
                if args.dont_update_ignore_list:
                    print(
                        f'{TERMINAL_YELLOW}"{file_name}" now has a correct copyright header - '
                        f'remove it from the ignore list or run this script without the --dont-update-ignore-list '
                        f'option to do this automatically!{TERMINAL_RESET}')
                else:
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument('-v', '--verbose', action='store_true', help='print more information (useful for debugging)')
    parser.add_argument('-r', '--replace', action='store_true', help='tries to update copyright notices')
    parser.add_argument(
        '-m', '--max-lines', type=int, default=30, help='how far to check for copyright notice in a file (default 30)')
    parser.add_argument(
        '-f',
        '--fuzzy-ratio',
        type=int,
...
def main() -> None:
    ...
    if wrong_header_files:
        ...
        for wrong_file in wrong_header_files:
            ...
            print(wrong_file)
    if must_be_updated:
        print(f'{TERMINAL_RED}Some files have no copyright and need to be added license header:{TERMINAL_RESET}')
    ...
    print(
        f'\n{TERMINAL_GREEN}Successfuly processed {num_files_processed} '
        f'file{"s"[:num_files_processed^1]}.{TERMINAL_RESET}\n')
