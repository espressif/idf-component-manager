...
Other environment variables (``IDF_COMPONENT_STORAGE_URL``, ``IDF_COMPONENT_REGISTRY_URL``, ``IDF_COMPONENT_API_TOKEN``, ``IDF_COMPONENT_DEFAULT_NAMESPACE``) override the corresponding profile field when set, so they can be used as deliberate overrides in CI or test environments.
