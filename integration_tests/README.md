...
| `RUN_INTEGRATION_TESTS` | Set to `1` to force running integration tests (v1), `2` to force running integration tests v1 + v2 (build system v2), or `0` to skip them. By default, integration tests run on the main branch, release branches, scheduled pipelines, or when `integration_tests` files are modified.                  |
