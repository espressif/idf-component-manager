...
def test_create_project_from_example_path(tmp_path):
    res = project_action(str(tmp_path), 'create-project-from-example', '-p', str(tmp_path), 'example/cmp=3.3.8:cmp_ex')
...
def test_create_project_from_example_no_path(tmp_path):
    example_name = 'cmp_ex'
    res = project_action(str(tmp_path), 'create-project-from-example', 'example/cmp=3.3.8:{}'.format(example_name))
...
def test_create_project_from_example_project_dir(tmp_path):
    example_name = 'cmp_ex'
    res = project_action(
        str(tmp_path), '--project-dir', str(tmp_path), 'create-project-from-example',
