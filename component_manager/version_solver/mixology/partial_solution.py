...
class PartialSolution:
    ...
    def unsatisfied(self):  # type: () -> List[Dependency]
        return [term.dependency for term in self._positive.values() if term.dependency.name not in self._decisions]
    ...
    def derive(self, dependency, is_positive, cause):  # type: (Dependency, bool, Incompatibility) -> None
        """
    ...
    def satisfier(self, term):  # type: (Term) -> Assignment
        ...
        for assignment in self._assignments:
            ...
            if (not assignment.dependency.is_root and not assignment.dependency.name == term.dependency.name):
                if not assignment.is_positive():
                    continue

                assert not term.is_positive()  # nosec
