...
class VersionSolver(object):
    ...
    def solve_manifest(self, manifest):  # type: (Manifest) -> None
        for requirement in manifest.dependencies:  # type: ComponentRequirement
            ...
            if (
                not isinstance(requirement.source, LocalSource)
                and requirement.name in self._local_root_requirements
            ):
            ...
            self._source.root_dep(
                Package(_requirement.name, _requirement.source), _requirement.version_spec
            )
            try:
