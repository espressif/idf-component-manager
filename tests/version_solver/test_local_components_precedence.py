...
def test_override_notice_is_not_duplicated(project_structure, recording_log):
    main_dir, components_dir = project_structure
    overrides_dir = main_dir.parent / 'overrides'
    overrides_dir.mkdir()
    ...
    override_messages = [
        record.message
        for record in recording_log.records
...
def test_unused_override_warning(project_structure, recording_log):
    """
    ...
    assert 'Override for "espressif/nonexistent" was not used' in recording_log.text


def test_unused_short_override_warning_is_not_duplicated(project_structure, recording_log):
    """
    ...
    unused_messages = [
        record.message for record in recording_log.records if 'was not used' in record.message
    ]
...
def test_local_components_dir_shadows_override(project_structure, recording_log):
    ...
    assert 'takes precedence' in recording_log.text
    assert 'Applying override for dependency "test/comp_b"' not in recording_log.text
