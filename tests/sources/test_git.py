...
def test_git_repo_root_no_name_warning(monkeypatch, tmp_path, caplog, git_path):
...
def test_git_repo_root_warns_on_install_dir_mismatch(monkeypatch, tmp_path, caplog):
    ...
    assert 'Component name "my_cmp" doesn\'t match the directory name "other_name"' in caplog.text
