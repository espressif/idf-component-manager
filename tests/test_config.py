...
from idf_component_tools.config import Config, ConfigError, ConfigManager, component_registry_url, get_api_url
...
def test_config_validation():
    ...
    with raises(ConfigError):
        Config({
            'profiles': {
                'in_office': {
                    'registry_url': 'pptp://api.localserver.local:5000/'
                },
            }
        }).validate()
...
def test_config_manager_validation(fixtures_path):
    manager = ConfigManager(path=os.path.join(fixtures_path, 'config', 'invalid_schema', 'idf_component_manager.yml'))
